...
<head>
    ...
    <style>
        /* Creates a muted background overlay fixed behind everything */
        body::before {
            content: "";
            position: fixed;
            top: 0;
            ...
            width: 100%;
            height: 100%;
            z-index: -1; /* Puts it completely behind your text/cards */
            background-image: url('{{ asset("background.jpeg") }}');
            background-size: cover;
            background-position: center;
            background-repeat: no-repeat;
            opacity: 0.50; /* Set intensity to exactly 50% capacity */
        }
    </style>
</head>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="bg-white shadow-md rounded-lg border border-gray-200 p-6 h-fit">
                ...
                <form action="{{ route('tickets.store') }}" method="POST" class="space-y-4">
                    ...
                    <button type="submit" class="w-full bg-emerald-700 hover:bg-emerald-800 text-white font-semibold text-sm py-2.5 px-4 rounded transition-colors shadow shadow-blue-900/20">
